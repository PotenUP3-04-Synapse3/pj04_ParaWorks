...
class MailDocumentProjectRouter(Protocol):
    def route(
        ...
        *,
        ...
        projects: list[MailDocumentProjectOption],
        ...
        for candidate in candidates:
            ...
            if project is None:
                decisions.append(
                    MailDocumentProjectRoutingDecision(
                        ...
                        alternatives=[option.project_key for option in projects[:3]],
            ...
            decisions.append(
                MailDocumentProjectRoutingDecision(
                    ...
                    assignment_summary=f'{candidate.title} 후보가 {project.name} 프로젝트와 연결됩니다.',
                    assignment_reason='후보의 제목, 요약, 증거 문구가 등록 프로젝트의 이름 또는 설명과 일치합니다.',
                    alternatives=[
                        option.project_key
                        for option in projects
                        if option.project_key != project.project_key
                    ][:3],
...
def create_mail_document_agent_review_items(
    *,
    ...
    project_router: MailDocumentProjectRouter | None = None,
) -> list[ReviewItem]:
    """
...
def project_route_mail_document_candidates(
    *,
    ...
    if not projects or not candidates:
        return MailDocumentProjectRoutingResult(
            decisions=[],
            model_name=getattr(router_model, 'model_name', 'deterministic-mail-document-project-router'),
        )
...
def _load_project_options(db: Session) -> list[MailDocumentProjectOption]:
...
def _best_project_match(
    ...
    projects: list[MailDocumentProjectOption],
) -> MailDocumentProjectOption | None:
    haystack = ' '.join(
        [
            candidate.source_id,
            candidate.title,
...
def _project_match_terms(project: MailDocumentProjectOption) -> list[str]:
    terms = {
    ...
    for token in project.summary.lower().replace(',', ' ').replace('.', ' ').split():
        if len(token) >= 4:
            terms.add(token)

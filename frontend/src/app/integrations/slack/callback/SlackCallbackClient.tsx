...
export function SlackCallbackClient() {
  ...
  useEffect(() => {
    if (slackError) {
      setCallbackState({
        status: "error",
        message: `Slack 인증이 취소되었거나 실패했습니다. (${slackError})`,
      });
      return;
    }
    ...
    if (missingCallbackParams) {
      setCallbackState({
        status: "error",
        message: "Slack 연결 정보를 확인할 수 없습니다.",
      });
      return;
    }
    ...
    apiGet<IntegrationConnection>(`/api/v1/integrations/slack/oauth/callback?${query.toString()}`)
      .then((connection) => {
        if (active) {
  ...
  }, [code, missingCallbackParams, slackError, state]);

  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <div>
        <p className="text-sm font-semibold text-[var(--workspace-rail-active)]">OAuth</p>
        <h2 className="mt-1 text-2xl font-semibold tracking-normal">Slack 연결 확인</h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--ink-muted)]">
          Slack 인증 결과를 ParaWorks 연결 상태로 저장합니다. 토큰 원문과 내부 token reference는 화면에 표시하지 않습니다.
        </p>
      </div>
...
function LoadingPanel() {
  return (
    <div className="flex items-start gap-3">
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[#fbf8fd] text-[#611f69]">
        <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" />
      </span>
      <div>
        <h3 className="text-base font-semibold">인증 정보를 확인하는 중</h3>
        <p className="mt-1 text-sm leading-6 text-[var(--ink-muted)]">
          Slack OAuth code를 backend callback으로 전달하고 연결 메타데이터만 저장합니다.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-base font-semibold">Slack 연결 완료</h3>
          <p className="mt-1 text-sm leading-6 text-[var(--ink-muted)]">
            {connection.workspace_name} 워크스페이스가 ParaWorks에 연결되었습니다.
          </p>
        </div>
      </div>
      ...
      <dl className="grid gap-3 rounded-lg bg-[#fbfaf8] p-4 text-sm sm:grid-cols-2">
        ...
        <div>
          <dt className="text-xs font-semibold text-[var(--ink-muted)]">Workspace ID</dt>
          <dd className="mt-1 break-all font-semibold">{connection.workspace_id}</dd>
        </div>
        <div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="text-base font-semibold">Slack 연결을 완료하지 못했습니다.</h3>
          <p className="mt-1 text-sm leading-6 text-red-700">{message}</p>
          <p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">
            Slack App의 Redirect URLs에 ParaWorks의 `SLACK_OAUTH_REDIRECT_URI` 값이 정확히 등록되어 있어야 합니다.
          </p>
        </div>
      </div>
...
function formatCallbackError(caught: unknown) {
  if (!(caught instanceof Error)) {
    return "Slack 연결 처리 중 알 수 없는 오류가 발생했습니다.";
  }

  if (caught.message.includes("invalid_code")) {
    return "Slack 인증 코드가 만료되었거나 이미 사용되었습니다. 연동 페이지에서 다시 연결해 주세요.";
  }

  if (caught.message.includes("redirect_uri")) {
    return "Slack App에 등록된 Redirect URL과 ParaWorks 설정값이 일치하지 않습니다.";
  }

  return "Slack 연결 처리 중 오류가 발생했습니다. Slack App 설정과 ParaWorks 환경 변수를 확인해 주세요.";
}
